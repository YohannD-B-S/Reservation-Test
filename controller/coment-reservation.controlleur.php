...
$commentText = ""; // Initialisation par défaut
...
if ($_SERVER["REQUEST_METHOD"] === "POST") { // si on reçoit une info via POST
    try {
        ...
        if (!is_null($reservationForUser)) { //si $reservationForUser n'est pas null 
            $reservationForUser->leaveCommand($commentText); // On appel la function leavCommand avec en parametre le commentaire
    ...
    require_once("../view/coment-reservation.view.php"); // Charger la vue
}
